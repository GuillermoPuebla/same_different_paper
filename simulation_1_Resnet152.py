...
# Model
def make_ResNet152_sketch_classifier(weights=None, trainable=True):
    # Inputs.
    image = Input((128, 128, 3))

    # Get CNN features.
    base_model = ResNet152(include_top=False, weights=None, input_tensor=image)
...
def make_ResNet152_sd_classifier(base_weights='imagenet', tuberlin_path=None, base_trainable=False, gap=True):
    # Define input tensor.
    image = Input((128, 128, 3))
    ...
    if base_weights == 'imagenet':
        ...
        sketch_b, sketch_m = make_ResNet152_sketch_classifier(weights=None, trainable=True)
...
def fine_tune_model(
    ...
    for i in range(n):
        ...
        model.fit(train_ds,
                  ...
                  validation_steps=validation_steps)
        # Save weights
        weights_name = save_name + 'ResNet152_instance_' + str(i) + '.hdf5'
...
def test_model_auc(ds, weights_dir, gap, model_name, condition):
    ...
    base_model, model = make_ResNet152_sd_classifier(base_weights=None, base_trainable=True, gap=gap)
...
def test_all_models_auc(ds, condition, weights_dir):
    ...
    ft_im_gap = test_model_auc(
        ds=ds,
        weights_dir=weights_dir+'/ResNet152_ImageNet_GAP',
        gap=True,
        model_name='ResNet152_ImageNet_GAP',
        condition=condition)
    ...
    ft_im_flatten = test_model_auc(
        ds=ds,
        weights_dir=weights_dir+'/ResNet152_ImageNet_flatten',
        gap=False,
        model_name='ResNet152_ImageNet_flatten',
        condition=condition)
    ...
    ft_tu_gap = test_model_auc(
        ds=ds,
        weights_dir=weights_dir+'/ResNet152_TUBerlin_GAP',
        gap=True,
        model_name='ResNet152_TUBerlin_GAP',
        condition=condition)
    ...
    ft_im_gap = test_model_auc(
        ds=ds,
        weights_dir=weights_dir+'/ResNet152_TUBerlin_flatten',
        gap=False,
        model_name='ResNet152_TUBerlin_flatten',
        condition=condition)
    results.extend(ft_im_gap)
...
if __name__ == '__main__':
    ...
    fine_tune_model(
        train_ds=train_ds,
        val_ds=val_ds,
        save_name='simulation_1/ResNet152_ImageNet_GAP/',
    ...
    df.to_csv('simulation_1/sim_1_resnet152_auc.csv')