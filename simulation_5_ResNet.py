...
def train_in_master_ds(
    ...
    fine_tune_model(
        train_ds=all_dss,
        val_ds=val_ds,
        save_name='simulation_5/ResNet_instances/',
...
def test_models_new_dss_auc(batch_size):
    ...
    conditions = ['Original', 'Regular', 'Irregular', 'Open', 'Wider Line', 'Scrambled', 'Random Color',
                  'Filled', 'Lines', 'Arrows', 'Rectangles', 'Straight Lines', 'Conected Squares', 'Circles']    
    ...
    for ds, condition in zip(datasets, conditions):
        ...
        ft_im_gap = test_model_auc(
            ds=ds,
            weights_dir='simulation_5/ResNet_instances',
            gap=True,
            model_name='ResNet',
            condition=condition)
...
if __name__ == '__main__':
    ...
    df = test_models_new_dss_auc(batch_size=112)
    df.to_csv('simulation_5/sim_5_auc.csv')
