...
def make_ResNet50_sd_classifier(base_weights='imagenet', base_trainable=False, gap=True):
    ...
    if base_weights == 'imagenet':
        ...
        sketch_m.load_weights('simulation1/ResNet50_TUBerlin_weights.hdf5')
...
def fine_tune_model(
    ...
    for i in range(n):
        # Define model.
        base_model, model = make_ResNet50_sd_classifier(base_weights=base_weights,
...
def test_all_models_auc(ds, condition):
    ...
    ft_im_gap = test_model_auc(
        ds=ds,
        weights_dir='simulation_1/ImageNet_GAP',
        gap=True,
        model_name='ResNet50_ImageNet_GAP',
        condition=condition)
    ...
    ft_im_flatten = test_model_auc(
        ds=ds,
        weights_dir='simulation_1/ImageNet_flatten',
        gap=False,
        model_name='ResNet50_ImageNet_flatten',
        condition=condition)
    ...
    ft_tu_gap = test_model_auc(
        ds=ds,
        weights_dir='simulation_1/TUBerlin_GAP',
        gap=True,
        model_name='ResNet50_TUBerlin_GAP',
        condition=condition)
    ...
    ft_im_gap = test_model_auc(
        ds=ds,
        weights_dir='simulation_1/TUBerlin_flatten',
        gap=False,
        model_name='ResNet50_TUBerlin_flatten',
        condition=condition)
    results.extend(ft_im_gap)
...
def test_all_models_all_ds_auc(batch_size):
    ...
    for ds, condition in zip(datasets, conditions):
        df = test_all_models_auc(ds, condition)
        dfs.append(df)
...
if __name__ == '__main__':
    ...
    # Train: TU-Berlin with GAP.
    fine_tune_model(
        train_ds=train_ds,
        val_ds=val_ds,
        save_name='simulation_1/TUBerlin_GAP/',
        ...
        n=10,
        lr=0.0001,
        base_weights='TU-Berlin',
        gap=True)

    # Train: TU-Berlin with flatten.
    fine_tune_model(
        train_ds=train_ds,
        val_ds=val_ds,
        save_name='simulation_1/TUBerlin_flatten/',
        epochs_top=EPOCHS_TOP,
        epochs=EPOCHS,
        steps_per_epoch=STEPS_PER_EPOCH,
        validation_steps=VALIDATION_STEPS,
    ...
    fine_tune_model(
        train_ds=train_ds,
        val_ds=val_ds,
        save_name='simulation_1/ImageNet_GAP/',
        epochs_top=EPOCHS_TOP,
        epochs=EPOCHS,
        steps_per_epoch=STEPS_PER_EPOCH,
        validation_steps=VALIDATION_STEPS,
    ...
    # Train: imagenet with flatten.
    fine_tune_model(
        train_ds=train_ds,
        val_ds=val_ds,
        save_name='simulation_1/ImageNet_flatten/',
        epochs_top=EPOCHS_TOP,
        epochs=EPOCHS,
        steps_per_epoch=STEPS_PER_EPOCH,
        validation_steps=VALIDATION_STEPS,
    ...
    df = test_all_models_all_ds_auc(batch_size=112)
    df.to_csv('simulation_1/sim_1_resnet_auc.csv')
